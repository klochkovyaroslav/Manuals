...
## Что бы убедиться, что файл id_rsa.pub соответствует файлу id_rsa

#### Используйте встроенную команду утилиты ssh-keygen, которая умеет считывать закрытый ключ и генерировать для него публичную часть:
```bash
ssh-keygen -y -f ~/.ssh/id_rsa
```

### Сравнение через отпечатки (Fingerprints)
...
```
Если строки вывода (алгоритм и SHA256-хэш) полностью совпали, то пара валидна.  
...
#### Перезагрузить хост
```bash
ssh -o ProxyCommand="nc -X 5 -x 192.168.21.4:1080 %h %p" 222.222.222.22 "echo 'ВАШ_ПАРОЛЬ' | sudo -S reboot"
```
#### Перезагрузить хост (с запросом пароля для sudo)
```bash
ssh -t -o ProxyCommand="nc -X 5 -x 192.168.21.4:1080 %h %p" 222.222.222.22 "sudo reboot"
```
