...
```
#### Детальная статистика по конкретной «тюрьме»
Чтобы увидеть количество заблокированных IP-адресов и их список для конкретного сервиса (например, SSH):  
...
```bash
sudo fail2ban-client ping (должен ответить pong)
```
